...
class EngineInput(OutputEvent):
    ...
    def is_valid_engine_input(engine_input):
        ...
        return 0. <= magnitude <= 1. and direction in EngineDirection

    def __init__(self, left_engine_input, right_engine_input):
        #assert EngineInput.is_valid_engine_input(left_engine_input), "{} is not a valid input for left engine"\
        #                                                             .format(left_engine_input)
        #assert EngineInput.is_valid_engine_input(right_engine_input), "{} is not a valid input for right engine"\
        #                                                              .format(right_engine_input)
